...
class TopologyAwareMCTS:
    ...
    def __init__(
        ...
        self.lam_ctrl      = LearnedLambdaController.from_hardcoded() if enable_dynamic_lambda else None
        ...
        # Mahalanobis model — loaded from calibration via set_thresholds().
        # None until first calibration result is received.
        # When present, _should_stop() uses this instead of the box thresholds above.
        self._mahal_model: Optional[MahalanobisEarlyStop] = None
    ...
    def set_thresholds(self, t: Dict):
        ...
        # Reconstruct Mahalanobis model from serialised dict if present.
        # This is the primary stopping criterion — box thresholds are fallback only.
        mahal_dict = t.get('mahal_model_dict')
        if mahal_dict and mahal_dict.get('ready'):
            self._mahal_model = MahalanobisEarlyStop.from_dict(mahal_dict)
        else:
            self._mahal_model = None

    # ── Simulation internals ──────────────────────────────────────────────────

    def _should_stop(self, m: Dict) -> bool:
        """
        Early-stop decision — prefers Mahalanobis model when available.

        Mahalanobis model (set via set_thresholds after calibration):
          Uses joint covariance of (H_v, G, Var_Q) fitted on probe positions.
          Fires when the current position is ≥ k σ from the mean in the
          resolved direction (low H_v, high G, low Var_Q simultaneously).
          Adapts to distribution shift as the network trains.

        Box threshold fallback (when no calibration has run yet):
          The conservative Strength-First defaults (0.15, 0.85, 0.01).
          These are intentionally tight — better to run extra sims than
          to stop a contested position prematurely.
        """
        H_v   = m['visit_entropy']
        G     = m['dominance_gap']
        Var_Q = m['value_variance']
        if self._mahal_model is not None:
            return self._mahal_model.should_stop(H_v, G, Var_Q)
        # Fallback: box thresholds
